...
public class RSAUtils { 
    ...
    public static RSAPublicKey getPublicKey(String publicKey) throws NoSuchAlgorithmException, InvalidKeySpecException { 
        ...
        X509EncodedKeySpec x509KeySpec = new X509EncodedKeySpec(Base64.getDecoder().decode(publicKey.getBytes())); 
        //generatePublic的参数是一个KeySpec的object
    ...
    public static RSAPrivateKey getPrivateKey(String privateKey) throws NoSuchAlgorithmException, InvalidKeySpecException {
        ...
        KeyFactory keyFactory = KeyFactory.getInstance(RSA_ALGORITHM); 
        PKCS8EncodedKeySpec pkcs8KeySpec = new PKCS8EncodedKeySpec(Base64.getDecoder().decode(privateKey.getBytes()));
        //generatePublic的参数是一个KeySpec的object
    ...
    public static String publicDecrypt(String data, RSAPublicKey publicKey){
        try{ 
            Cipher cipher = Cipher.getInstance(RSA_ALGORITHM); cipher.init(Cipher.DECRYPT_MODE, publicKey);
    ...
    private static byte[] rsaSplitCodec(Cipher cipher, int opmode, byte[] datas, int keySize){
         int maxBlock = 0;
        if(opmode == Cipher.DECRYPT_MODE){//解密模式
            maxBlock = keySize / 8;
         }else{//加密模式
        ...
        try{ 
            while(datas.length > offSet){ 
                if(datas.length-offSet > maxBlock){ 
                    ...
                    //从指定的buff字节数组的初始位置开始写入buff字节数组长度的字节到out中去
